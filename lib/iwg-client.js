...
class IwgClient {
                    ...
                    .forEach(key => {
                        ...
                        if (fs.existsSync(privateKeyFile)) {
                            try {
                                ...
                                if (config) {
                                    const privateKey = IwgClient.read(privateKeyFile).replace(os.EOL, '');
    ...
    static async generateClientKeys() {
        ...
        return new Promise((resolve) => {
            if (!fs.existsSync(keysDir + 'public.pem')) {
                ...
                // };
                IwgClient.keys = {
                    publicPem: IwgClient.read(keysDir + 'public.pem'),
                    private: IwgClient.read(keysDir + 'private.pem')
                };
            }
            ...
            const payload = {
                remotes: activePeers.map(peer => {
                    ...
                    if (peer.isAutoGenerateKeys) {
                        publicKey = IwgClient.read(self.publicKeyFileName(peer.ip)).replace(os.EOL, '');
                    } else {
    ...
    async createLocalPeer() {
        try {
            ...
            const publicKey = IwgClient.read(`${keysDir}publickey`).replace(os.EOL, '');
            IwgClient.keys.localPeerPrivateKey = IwgClient.read(`${keysDir}privatekey`).replace(os.EOL, '');
