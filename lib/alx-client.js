            ...
            const response = await HttpClient.post(HttpClient.composeAbsoluteUrl(`/clients/${AdapterProvider.id}/endpoints`, API_VERSION),
                {
                    ...
                    config: {
                        ...
                        functionFirst: AdapterProvider.get().config?.functionFirst,
                        concatWord: AdapterProvider.get().config?.concatWord,
                    }
                }
            );
            const discovered = JSON.stringify(response?.data?.endpoints)
            this.log.silly(`Discovered endpoints: ${discovered}`);
