...
class AlexaHandler {
    ...
    static async fetchEndpoints() {
        try {
            if (AlexaHandler.endpoints.length === 0) {
                ...
                let response = await HttpClient.get(HttpClient.composeAbsoluteUrl(`/clients/${AdapterProvider.id}/endpoints`, API_VERSION));
    ...
    static async postEndpoints(payload) {
        try {
            ...
            let response = await HttpClient.post(HttpClient.composeAbsoluteUrl(`/clients/${AdapterProvider.id}/sync_endpoints`, API_VERSION), payload);
            ...
            if (response.status < 400) {
                ...
                await AlexaHandler.updateSubscriptions(AlexaHandler.endpoints);
                // report last values of newly subscribed states, no need to wait for the promise to be resolved here
    ...
    static async reportStateChange(address, value) {
        try {
            ...
            let response = await HttpClient.put(HttpClient.composeAbsoluteUrl(`/clients/${AdapterProvider.id}/values`, API_VERSION), { address: address, value: value });
    ...
    */
    static stateChangeHandler(id, state) {
        ...
        if (state) {
            ...
            if (state.ack) {
                ...
                // check whether this state has a cool down which is already running or has to be started
                let capability = null
                ...
                if (capability) {
                    ...
                    if (coolingDown) {
                        log.silly(`skipped reporting state ${id} change due to cool down`);
                        return;
                    } else {
                        // start cool down
                        AlexaHandler.cooldowns.set(id, true)
                        // reset cool down after a predefined time
                        setTimeout(function() {
    ...
    static async capabilities() {
        try {
            ...
            log.error(`failed fetching capabilities`)
            log.error(e)
        }
        return null;
    }
    ...
    static async accounts() {
        try {
            ...
            let response = await HttpClient.post(HttpClient.composeAbsoluteUrl(`/clients/${AdapterProvider.id}/accounts`, API_VERSION), {});
