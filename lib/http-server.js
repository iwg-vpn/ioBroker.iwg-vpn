...
class HttpServer {
    ...
    async wellKnownPublicKey() {
        if (!this.serverPublicKey) {
            let response = await IwgClient.get(`/.well-known/public_key`);
                        ...
                        if (req.method == 'GET') {
                            ...
                            if (!whiteListedStates?.includes(id)) {
                                ...
                                if (!sig || !message || !(await this.isTrustworth(message, sig))) {
                                    ...
                                    throw new Error('not a trustworth api call')
                                }   
                            }
                            ...
                            if (newValue) {
                                ...
                                try {
                                    ...
                                    if (type == 'number') {
                                        const newValueAsNumner = parseInt(newValue)
                                        if (!isNaN(newValueAsNumner)) {
                                            newValue = newValueAsNumner
                                        }
                                    }
                                } catch {
                                    // nop
